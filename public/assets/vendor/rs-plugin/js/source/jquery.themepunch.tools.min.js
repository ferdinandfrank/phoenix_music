		...
		var TweenLite = _class("TweenLite", function(target, duration, vars) {
			...
			_autoCSS = function(vars, target) {
				var css = {},
					p;
				for (p in vars) {
					if (!_reservedProps[p] && (!(p in target) || p === "transform" || p === "x" || p === "y" || p === "width" || p === "height" || p === "className" || p === "border") && (!_plugins[p] || (_plugins[p] && _plugins[p]._autoCSS))) { //note: <images> elements contain read-only "x" and "y" properties. We should also prioritize editing css width/height rather than the element's properties.
						css[p] = vars[p];
						delete vars[p];
					}
				}
...
;(function (factory) {
    ...
    // Custom selector to find all `images` elements with a valid `src` attribute.
    $.expr[':']['has-src'] = function (obj) {
        // Ensure we are dealing with an `images` element with a valid
        // `src` attribute.
        return $(obj).is('images[src][src!=""]');
    };
        ...
        this.each(function () {
            ...
            if (waitForAll) {
                ...
                obj.find('*').addBack().each(function () {
                    ...
                    // If an `images` element, add it. But keep iterating in
                    // case it has a background image too.
                    if (element.is('images:has-src')) {
                        allImgs.push({

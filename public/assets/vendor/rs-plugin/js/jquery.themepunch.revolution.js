		...
		function initSlider(container,opt) {
						...
						if (container.data('aimg')!=undefined) {
							if ((container.data('aie8')=="enabled" && isIE(8)) || (container.data('amobile')=="enabled" && is_mobile()))
								container.html('<images class="tp-slider-alternative-image" src="'+container.data("aimg")+'">');
						}
...
********************************/
		...
		********************************/
		...
		var createBullets = function(container,opt) {
			...
			container.find('>ul:first >li').each(function(i) {
							var src=container.find(">ul:first >li:eq("+i+") images:first").attr('src');
		...
		*******************************************************/
		...
		var prepareSlides = function(container,opt) {
			...
			container.find('>ul:first >li >images').each(function(j) {
				...
				if (isIE(8)) {
					...
					pari.find('.tp-bgimg').append('<images class="ieeightfallbackimage defaultimg" src="'+src+'" style="width:100%">');
				}
						...
						for (var j=0;j<opt.slots;j++) {
							...
							for (var i=0;i<opt.slots;i++) 	{
								...
								if (isIE(8)) {
									...
									sh.find('.slot ').last().find('.slotslide').append('<images src="'+imgsrc+'">');
						 ...
						 if (vorh == "horizontal") {
							...
							for (var i=0;i<opt.slots;i++) {
									...
									if (scalestart!=undefined && rotatestart!=undefined)
										punchgs.TweenLite.set(sh.find('.slot').last(),{rotationZ:rotatestart});
									if (isIE(8)) {
									   sh.find('.slot ').last().find('.slotslide').append('<images class="ieeightfallbackimage" src="'+imgsrc+'" style="width:100%;height:auto">');
									   ieimgposition(sh,opt);
								}
							}
						} else {
		...
		********************************************/
		var loadAllPrepared = function(container,alreadyinload) {
			...
			container.find('images, .defaultimg').each(function(i) {
				var img = jQuery(this),
		...
		var waitForLoads = function(element,call,opt) {
			...
			var inter = setInterval(function() {
				...
				opt.cd=0;
				 var found = 0;
				 element.find('images, .defaultimg').each(function(i) {
		...
		var nextli = container.find('>ul:first-child >li:eq('+opt.next+')'),
			...
			nextsh.find('.defaultimg').each(function() {
				...
				if (nextsh.find('.kenburnimg').length==0)
					nextsh.append('<div class="kenburnimg" style="position:absolute;z-index:1;width:100%;height:100%;top:0px;left:0px;"><images src="'+defimg.attr('src')+'" style="-webkit-touch-callout: none;-webkit-user-select: none;-khtml-user-select: none;-moz-user-select: none;-ms-user-select: none;user-select: none;position:absolute;width:'+imgobj.w+'%;height:'+imgobj.h+'%;"></div>');
				else {
					nextsh.find('.kenburnimg images').css({width:imgobj.w+'%',height:imgobj.h+'%'});
				}



				var kbimg = nextsh.find('.kenburnimg images');
		...
		********************************/
		var stopKenBurn = function(container,opt) {
			...
			container.find('.defaultimg').each(function() {
				...
				punchgs.TweenLite.killTweensOf(defimg.data('kenburn images'),false);
				if (defimg.data('kenburn') != undefined) {
					defimg.data('kenburn').pause();
				}
				if (defimg.data('currotate') != undefined && defimg.data('bgposition') !=undefined && defimg.data('curscale') != undefined)
					punchgs.TweenLite.set(defimg,{rotation:defimg.data('currotate'), backgroundPosition:defimg.data('bgposition'), backgroundSize:defimg.data('curscale')});
				if (defimg!= undefined && defimg.data('kenburn images') != undefined && defimg.data('kenburn images').length>0) punchgs.TweenLite.set(defimg.data('kenburn images'),{autoAlpha:0});
		...
		var createThumbs = function(container,opt) {
			...
			container.find('>ul:first >li').each(function(i) {
							...
							if (li.data('thumb') !=undefined)
								...
								var src=li.find("images:first").attr('src');
